...
- **Open Settings → Widgets once** after login (or switching session) so shared storage includes `sessionId`. If it’s missing, rows may be **non-tappable** (Android) or taps may do nothing useful (iOS).
...
- **Tap opens login, or nothing happens on tap:** open **Settings → Widgets** while logged in so setups sync with the current **session id**; try again. Push/other `directus://` links without `?sessionId=` are ignored for safety—prefer widget row taps or links that include the session query.
